...
function BeveragePreferenceServiceClient:getBeverage(beverage_type)
  self:send_getBeverage(beverage_type)
  return self:recv_getBeverage(beverage_type)
end

function BeveragePreferenceServiceClient:send_getBeverage(beverage_type)
...
function BeveragePreferenceServiceClient:recv_getBeverage(beverage_type)
...
function BeveragePreferenceServiceProcessor:process_getBeverage(seqid, iprot, oprot, server_ctx)
  ...
  iprot:readMessageEnd()
  local result = getBeverage_result:new{}
  local status, res = pcall(self.handler.getBeverage, self.handler, args.beverage_type)
  if not status then
  ...
  while true do
    ...
    elseif fid == 1 then
      if ftype == TType.I32 then
        self.beverage_type = iprot:readI32()
      else
        iprot:skip(ftype)
      end
    else
...
function getBeverage_args:write(oprot)
  ...
  if self.beverage_type ~= nil then
    oprot:writeFieldBegin('beverage_type', TType.I32, 1)
    oprot:writeI32(self.beverage_type)
    oprot:writeFieldEnd()
  end
